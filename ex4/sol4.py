...
def getRespons(M, imShape):
    # print(imShape)
    print(M.shape)
    for row in range(imShape[ROWS]):
        for col in range (imShape[COLS]):
            mPerPoint = [[M[0][0][ROWS][COLS], M[0][1][ROWS][COLS]],
                         [M[1][0][ROWS][COLS], M[1][1][ROWS][COLS]]]
            eigenValues, eigenVectores = np.linalg.eig(mPerPoint)
