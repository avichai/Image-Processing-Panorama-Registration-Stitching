...
def generate_panorama(data_dir, file_prefix, num_images, figsize=(20, 20)):
    ...
    files = [os.path.join(data_dir, '%s%d.jpg' % (file_prefix, i + 1)) for i in
             range(num_images)]
    ...
    p_d = [sol4.find_features(sol4.build_gaussian_pyramid(im, 3, 3)) for im in
           ims]  # todo fix if nedded
    ...
    for i in range(num_images - 1):
        ...
        sol4.display_matches(ims[i], ims[i + 1], points1, points2, inliers)
        Hs.append(H12)
    ...
    ims_rgb = [sol4.read_image(f, 2) for f in files]
